...
policy.generateHtml = function (label, fieldName, objName, objId, index) {
  return '<div data-index="' + index + '" class="data-' + fieldName + '">'
      ...
      + 'id="parent-' + fieldName + '-' + index + '-policyBitList">'
      + ''
      + '    <button type="button" class="btn btn-primary btn-xs" '
      + 'name="add-' + fieldName + '-' + index + '-policyBitList">'
      + '      <span class="glyphicon glyphicon-plus"></span>'
      + '      <span> Policy bit</span>'
      + '    </button>'
      + ''
      + '</div>'
      + '<hr>'
...
policy.add = function (label, fieldName, objName, objId) {
  ...
  buttonAdd.click(function (e) {
    ...
    let me = $(this),
        obj = $('.data-' + fieldName),
        index = obj.length,
        html = policy.generateHtml(label, fieldName, objName, objId, index);

    me.before(html);
    policy.add(label, fieldName, objName, objId);
    policy.addEvent(fieldName, objName, objId);
  });
  ...
  buttonRemove.click(function (e) {
    ...
    policy.reorderIndexes(fieldName, objName, objId, index);
    parent.remove();
    policy.addEvent(fieldName, objName, objId);
  });
...
policy.reorderIndexes = function (fieldName, objName, objId, index) {
...
policy.addEvent = function (fieldName, objName, objId) {
  let obj = $('.data-' + fieldName);
  for (let i = 0; i < obj.length; i++) {
    policyBit.add('Policy bit', fieldName + '-' + i + '-policyBitList',
        objName + '[' + i + '].policyBitList',
        objId + i + '.policyBitList');
  }
};
