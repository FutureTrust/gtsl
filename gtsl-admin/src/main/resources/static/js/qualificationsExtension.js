...
let qualificationsExtension = {};

qualificationsExtension.generateHtml = function (label, fieldName, objName, objId, index) {
  return '<div data-index="' + index + '" class="data-' + fieldName + '">'
      ...
      + 'name="add-' + fieldName + '-' + index + '-qualifiers">'
      ...
      + '  <hr>'
      + ''
      + criteria.generateHtml('Criteria',  fieldName + '-' + index + '-criteria', objName + '[' + index + '].criteria', objId + index + '.criteria')
      + ''
      + '</div>'
      + ''
      + ''
...
qualificationsExtension.add = function (label, fieldName, objName, objId) {
  ...
  buttonAdd.click(function (e) {
    ...
    let me = $(this),
        obj = $('.data-' + fieldName),
        index = obj.length,
        html = qualificationsExtension.generateHtml(label, fieldName, objName, objId, index);

    me.before(html);
    qualificationsExtension.add(label, fieldName, objName, objId);
    qualificationsExtension.addEvent(fieldName, objName, objId);
  });
  ...
  buttonRemove.click(function (e) {
    ...
    qualificationsExtension.reorderIndexes(fieldName, objName, objId, index);
    parent.remove();
    qualificationsExtension.addEvent(fieldName, objName, objId);
  });
...
qualificationsExtension.reorderIndexes = function (fieldName, objName, objId, index) {
...
qualificationsExtension.addEvent = function (fieldName, objName, objId) {
  let obj = $('.data-' + fieldName);
  for (let i = 0; i < obj.length; i++) {
    listString.add('Qualifier', fieldName + '-' + i + '-qualifiers',
        objName + '[' + i + '].qualificationList',
        objId + i + '.qualificationList');
    criteria.add('Criteria', fieldName + '-' + i + '-criteria',
        objName + '[' + i + '].criteria',
        objId + i + '.criteria');
  }
};
