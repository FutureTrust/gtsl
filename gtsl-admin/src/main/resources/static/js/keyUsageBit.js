...
keyUsageBit.generateHtml = function (label, fieldName, objName, objId, index) {
  return '<div data-index="' + index + '" class="data-' + fieldName + '">'
      ...
      + ''
      + '               <div>'
      + '                 <input id="' + objId + index + '.value1" '
      + 'name="' + objName + '[' + index + '].value" '
      + 'value="true" '
      + 'checked="checked" '
      + 'type="checkbox">'
      + '                 <input name="_' + objName + '[' + index + '].value" value="on" type="hidden">'
      + '                 <label> Value</label>'
      + '               </div>'
      + ''
      + '               <div>'
...
keyUsageBit.add = function (label, fieldName, objName, objId) {
  ...
  buttonAdd.click(function (e) {
    ...
    let me = $(this),
        obj = $('.data-' + fieldName),
        index = obj.length,
        html = keyUsageBit.generateHtml(label, fieldName, objName, objId, index);

    me.before(html);
    keyUsageBit.add(label, fieldName, objName, objId);
    keyUsageBit.addEvent(fieldName, objName, objId);
  });
  ...
  buttonRemove.click(function (e) {
    ...
    keyUsageBit.reorderIndexes(fieldName, objName, objId, index);
    parent.remove();
    keyUsageBit.addEvent(fieldName, objName, objId);
  });
...
keyUsageBit.reorderIndexes = function (fieldName, objName, objId, index) {
...
keyUsageBit.addEvent = function (fieldName, objName, objId) {
  let obj = $('.data-' + fieldName);
};
