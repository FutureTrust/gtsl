...
criteriaList.generateHtml = function (label, fieldName, objName, objId, index) {
  return '<div data-index="' + index + '" class="data-' + fieldName + '">'
      ...
      + ''
      + criteria.generateHtml('Criteria',  fieldName + '-' + index, objName + '[' + index + ']', objId + index)
      + ''
...
criteriaList.add = function (label, fieldName, objName, objId) {
  ...
  buttonAdd.click(function (e) {
    ...
    let me = $(this),
        obj = $('.data-' + fieldName),
        index = obj.length,
        html = criteriaList.generateHtml(label, fieldName, objName, objId, index);

    me.before(html);
    criteriaList.add(label, fieldName, objName, objId);
    criteriaList.addEvent(fieldName, objName, objId);
  });
  ...
  buttonRemove.click(function (e) {
    ...
    criteriaList.reorderIndexes(fieldName, objName, objId, index);
    parent.remove();
    criteriaList.addEvent(fieldName, objName, objId);
  });
...
criteriaList.reorderIndexes = function (fieldName, objName, objId, index) {
...
criteriaList.addEvent = function (fieldName, objName, objId) {
  let obj = $('.data-' + fieldName);
  for (let i = 0; i < obj.length; i++) {
    criteria.add('Criteria', fieldName + '-' + i,
        objName + '[' + i + ']',
        objId + i);
  }
};
