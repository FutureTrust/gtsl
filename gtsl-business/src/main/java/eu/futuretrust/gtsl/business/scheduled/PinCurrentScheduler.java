...
@Component
public class PinCurrentScheduler extends AbstractPinScheduler {

  private static final Logger LOGGER = LoggerFactory.getLogger(PinCurrentScheduler.class);

  @Autowired
  public PinCurrentScheduler(
  ...
  public void run() {
    scheludePin(commitAddress -> {
      try {
        return storageService.pinCurrentVersion(commitAddress);
      } catch (Exception e) {
    ...
    }, ipfsProperties.getCronCurrent().isEnabled());
  }

  @Override
  public void initializeScheduler() {
    this.reSchedule(ipfsProperties.getCronCurrent());
  }
